...
// 
    ...
    struct operation_visitor final {
        operation_visitor(
            ...
            const golos::plugins::operation_history::operation_object& op_obj,
        ...
        const golos::plugins::operation_history::operation_object& op;
        std::string account;
        ...
        void operator()(Op &&) const {
            ...
            auto itr = idx.lower_bound(boost::make_tuple(account, uint32_t(-1)));
            uint32_t sequence = 0;
    ...
    public:
        ...
        void on_operation(const golos::chain::operation_notification& note) {
            ...
            const auto& idx = db.get_index<operation_history::operation_index>().indices().get<by_id>();
            auto itr = idx.find(operation_history::operation_id_type(note.db_id));
            ...
            for (const auto& item : impacted) {
                auto fitr = tracked_accounts.lower_bound(item);
                if (!tracked_accounts.size() ||
                    (fitr != tracked_accounts.end() && fitr->first <= item && item <= fitr->second)
                ) {
                    note.op.visit(operation_visitor(db, note, *itr, item));
                }
            }
        }
    ...
    void plugin::plugin_initialize(const boost::program_options::variables_map& options) {
        ...
        pimpl->database().post_apply_operation.connect([&](const golos::chain::operation_notification &note){
