        ...
        vector<optional<account_object>> database_api_impl::lookup_account_names(const vector<string> &account_names) const {
            ...
            std::transform(account_names.begin(), account_names.end(), std::back_inserter(result),
                    [&accounts_by_name](const string &name) -> optional<account_object> {
                        ...
                        return itr == accounts_by_name.end()
                               ? optional<account_object>() : *itr;
                    });
            return result;
        }
...
//   idump((start_parent_author)(start_permlink)(limit) );
            ...
            while (itr != last_update_idx.end() && result.size() < limit) {
                if (filter_by_parent_author &&
                    itr->parent_author != start_parent_author) {
                    return result;
                }
...
//   idump((query));
        ...
        vector<discussion> database_api::metadata_filter(const vector<discussion> &discussions,
                const set<string> tags) const {
            vector<discussion> return_vector;

            for (vector<discussion>::value_type iterator : discussions) {
                tags::comment_metadata meta;

                if(iterator.json_metadata.size()) {
                    try {
                        meta = fc::json::from_string(iterator.json_metadata).as<tags::comment_metadata>();
                    } catch(const fc::exception& e) {
        ...
        vector<discussion> database_api::get_discussions_by_trending(const discussion_query &query) const {
            ...
            return query.filter_tags.empty() ?
                   get_discussions(query, tag, parent, tidx, tidx_itr, [](const comment_object &c) {
                       return c.children_rshares2 <= 0 || c.mode != first_payout;
                   }) :
                   metadata_filter(get_discussions(query, tag, parent, tidx, tidx_itr, [](const comment_object &c) {
                       return c.children_rshares2 <= 0 || c.mode != first_payout;
                   }), query.filter_tags);
        }
        ...
        vector<discussion> database_api::get_discussions_by_created(const discussion_query &query) const {
            ...
            return query.filter_tags.empty() ?
                   get_discussions(query, tag, parent, tidx, tidx_itr) :
                   metadata_filter(get_discussions(query, tag, parent, tidx, tidx_itr), query.filter_tags);
        }
        ...
        vector<discussion> database_api::get_discussions_by_feed(const discussion_query &query) const {
            ...
            return query.filter_tags.empty() ? result : metadata_filter(result, query.filter_tags);
        }
                ...
                if (part[0].size() && part[0][0] == '@') {
                    ...
                    if (part[1] == "transfers") {
                        ...
                        while (itr != pidx.end() && itr->author == acnt &&
                            ...
                            set_pending_payout(_state.content[acnt + "/" +
                                                              itr->permlink]);
                            ++itr;
                            ++count;
                        }
                        ...
                        while (itr != pidx.end() && itr->author == acnt &&
                               count < 20 && !itr->parent_author.size()) {
