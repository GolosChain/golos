...
BOOST_AUTO_TEST_CASE(worker_techspec_validate) {
    ...
    BOOST_TEST_MESSAGE("-- Zero payments interval case");

    CHECK_PARAM_INVALID(op, payments_interval, 0);

    BOOST_TEST_MESSAGE("-- Single payment with too big interval case");

    op.payments_count = 1;
    CHECK_PARAM_INVALID(op, payments_interval, 60);

    BOOST_TEST_MESSAGE("-- Single payment with normal interval case");

    op.payments_count = 1;
    CHECK_PARAM_VALID(op, payments_interval, 1);
}
...
BOOST_AUTO_TEST_CASE(worker_techspec_apply_create) {
    ...
    BOOST_CHECK_EQUAL(wto.created, now);
    ...
    BOOST_CHECK_EQUAL(wto.completion_date, fc::time_point_sec::min());
...
BOOST_AUTO_TEST_CASE(worker_techspec_apply_modify) {
    ...
    BOOST_TEST_MESSAGE("-- Normal worker techspec edit case");

    op.worker_proposal_author = "alice";
    op.worker_proposal_permlink = "alice-proposal";
    op.payments_interval = 1;
    op.payments_count = 1;
    BOOST_CHECK_NO_THROW(push_tx_with_ops(tx, bob_private_key, op));
    generate_block();
...
BOOST_AUTO_TEST_CASE(worker_techspec_apply_multiple) {
    BOOST_TEST_MESSAGE("Testing: worker_techspec_apply_multiple");
    ...
    BOOST_TEST_MESSAGE("-- Creating alice-proposal");
    ...
    BOOST_TEST_MESSAGE("-- Creating dave-proposal");

    comment_create("dave", dave_private_key, "dave-proposal", "", "dave-proposal");

    worker_proposal("dave", dave_private_key, "dave-proposal", worker_proposal_type::task);
    generate_block();

    BOOST_TEST_MESSAGE("-- Creating bob-techspec for alice-proposal");
    ...
    const auto& bob_post_id = db->get_comment("bob", string("bob-techspec")).id;

    BOOST_TEST_MESSAGE("-- Creating carol-techspec for alice-proposal");
    ...
    const auto& carol_post_id = db->get_comment("carol", string("carol-techspec")).id;

    BOOST_TEST_MESSAGE("-- Creating eve-techspec for dave-proposal");

    comment_create("eve", eve_private_key, "eve-techspec", "", "eve-techspec");

    wtop.author = "eve";
    wtop.permlink = "eve-techspec";
    wtop.worker_proposal_author = "dave";
    wtop.worker_proposal_permlink = "dave-proposal";
    BOOST_CHECK_NO_THROW(push_tx_with_ops(tx, eve_private_key, wtop));
    generate_block();

    const auto& eve_post_id = db->get_comment("eve", string("eve-techspec")).id;

    BOOST_TEST_MESSAGE("-- Checking all");

    {
        const auto& idx = db->get_index<worker_techspec_index>().indices().get<by_id>();
        auto itr = idx.begin();
        BOOST_CHECK(itr != idx.end());
        BOOST_CHECK_EQUAL(itr->post, bob_post_id);
        BOOST_CHECK_EQUAL(db->get_comment(itr->worker_proposal_post).author, "alice");
        itr++;
        BOOST_CHECK(itr != idx.end());
        BOOST_CHECK_EQUAL(itr->post, carol_post_id);
        BOOST_CHECK_EQUAL(db->get_comment(itr->worker_proposal_post).author, "alice");
        itr++;
        BOOST_CHECK(itr != idx.end());
        BOOST_CHECK_EQUAL(itr->post, eve_post_id);
        BOOST_CHECK_EQUAL(db->get_comment(itr->worker_proposal_post).author, "dave");
    }
}
...
BOOST_AUTO_TEST_CASE(worker_techspec_delete_apply) {
    ...
    BOOST_TEST_MESSAGE("-- Deleting worker techspec with non-existant post case");
    ...
    BOOST_TEST_MESSAGE("-- Deleting non-existant worker techspec case");
    ...
    BOOST_TEST_MESSAGE("-- Checking cannot delete post with worker techspec");
    ...
    wtop.worker_proposal_author = "alice";
    wtop.worker_proposal_permlink = "alice-proposal";
    wtop.specification_cost = ASSET_GOLOS(6000);
    wtop.development_cost = ASSET_GOLOS(60000);
    wtop.payments_interval = 60;
    wtop.payments_count = 2;
    BOOST_CHECK_NO_THROW(push_tx_with_ops(tx, bob_private_key, wtop));
    generate_block();
    ...
    const auto* wto_del = db->find_worker_techspec(wto_post.id);
    BOOST_CHECK(!wto_del);
...
BOOST_AUTO_TEST_CASE(worker_techspec_approve_apply_top19_updating) {
    BOOST_TEST_MESSAGE("Testing: worker_techspec_approve_apply_top19_updating");
...
BOOST_AUTO_TEST_CASE(worker_techspec_approve_apply_final) {
    BOOST_TEST_MESSAGE("Testing: worker_techspec_approve_apply_final");
    ...
    auto consumption = std::min(cost * 60*60*24*30 / secs, cost);
    BOOST_CHECK_EQUAL(db->get_dynamic_global_properties().worker_consumption_per_month, consumption);
    ...
    BOOST_TEST_MESSAGE("-- Checking cannot repeat approve same techspec");
    ...
    BOOST_TEST_MESSAGE("-- Checking cannot edit worker proposal with approved techspec");

    worker_proposal_operation wpop;
    wpop.author = "alice";
    wpop.permlink = "alice-proposal";
    GOLOS_CHECK_ERROR_LOGIC(cannot_edit_worker_proposal_with_approved_techspec, alice_private_key, wpop);

    BOOST_TEST_MESSAGE("-- Checking cannot edit approved worker techspec");

    wtop.author = "bob";
    wtop.permlink = "bob-techspec";
    wtop.payments_count = 2;
    GOLOS_CHECK_ERROR_LOGIC(this_worker_proposal_already_has_approved_techspec, bob_private_key, wtop);

    BOOST_TEST_MESSAGE("-- Checking subtracting consumption when deleting approved worker techspec");
    ...
    BOOST_CHECK_EQUAL(db->get_dynamic_global_properties().worker_consumption_per_month, ASSET_GOLOS(0));
}

BOOST_AUTO_TEST_CASE(worker_techspec_approve_apply_final_disapprove) {
    BOOST_TEST_MESSAGE("Testing: worker_techspec_approve_apply_final_disapprove");
    ...
    BOOST_TEST_MESSAGE("-- Checking can edit worker proposal with closed techspec");
    ...
    BOOST_TEST_MESSAGE("-- Checking can edit closed worker techspec");
...
BOOST_AUTO_TEST_CASE(worker_techspec_approve_apply_clear_on_approve) {
    ...
    db->set_clear_old_worker_techspec_approves(true);
